...
import net.preibisch.bigdistributor.algorithm.blockmanagement.blockinfo.ComplexBlockInfo;
import net.preibisch.bigdistributor.algorithm.errorhandler.logmanager.MyLogger;
import net.preibisch.bigdistributor.algorithm.multithreading.ImagePortion;
...
public class Block extends AbstractInterval
{
	...
	public int hashCode() {
		final int prime = 31;
		int result = 1;
		result = prime * result + Arrays.hashCode(effectiveOffset);
		return result;
	}
	...
	public boolean equals(Object obj) {
		...
		if (!Arrays.equals(effectiveOffset, other.effectiveOffset))
			return false;
		return true;
	}
			...
			final boolean isPrecise )
	{
		...
		this.portionsCopy = ImagePortion.divideIntoPortions( n );
		...
		// split up into many parts for multithreading
		this.portionsPaste = ImagePortion.divideIntoPortions( n );
	...
	public void copyBlock( final RandomAccessible< FloatType > source, final RandomAccessibleInterval< FloatType > block )
	{
		...
		{
			MyLogger.log().error( "Failed to copy block: " + e);
			return;
		}
	}
	...
	public void pasteBlock( final RandomAccessibleInterval< FloatType > target, final RandomAccessibleInterval< FloatType > block)
	{
		...
		{
			MyLogger.log().error( "Failed to paste block: " + e );
