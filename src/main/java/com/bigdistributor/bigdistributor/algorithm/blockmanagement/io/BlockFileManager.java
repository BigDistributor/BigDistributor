package net.preibisch.bigdistributor.algorithm.blockmanagement.io;
...
import net.preibisch.bigdistributor.algorithm.blockmanagement.block.Block;
...
import net.preibisch.bigdistributor.io.IOTools;
...
public class BlockFileManager {
	...
	public static void saveAllBlocks(RandomAccessibleInterval<FloatType> image, List<Block> blocks,
			File folder, String baseName, DataExtension extension) {
		final RandomAccessible<FloatType> infiniteImg = Views.extendMirrorSingle(image);
		Integer i = 0;
		for (final Block block :blocks) {
			++i;
//			extension.file(i.toString())
			File f = new File(folder, baseName+i+".tif");
